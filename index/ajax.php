...
{	header('Content-Type:text/plain; charset=utf-8');
	...
	$carcount = "<p>目前有 <span class='counts'>".$count1."</span> 辆二手车供您选择</p><p>累计出售<span class='counts'><a href='/index.php?m=issell&amp;c=s_2'> ".$count3."</a> </span>辆 今天新增 <span class='counts'>".$count2."</span> 辆</p>";
	echo $carcount;
	exit;
}
//车源点击
...
//热门租车车源排行
if(!empty($_GET['ajax']) && isset($_GET['cartype']) && $_GET['cartype']=="renthot"){
	header('Content-Type:text/plain; charset=utf-8');

	$where = "ishot=1 and p_mainpic!='' ";
	if(!isset($_COOKIE['city']) or empty($_COOKIE['city'])){
		$_COOKIE['city'] = 0;
	}
	$list = get_rentcarlist($_COOKIE['city'],$where,'10','listtime desc');
	$str = "";
	foreach($list as $key => $value) {
		$str .= "<div class='hotcarlist'><a href=".$value['p_url']." target='_blank'><img src=".$value['p_mainpic']."></a>
			<p class='mt5'><a href=".$value['p_url']." target='_blank' >".$value['p_shortname']."</a></p>
			<p><span class='orange01 fb'>".$value['dayprice']."</span></p>
		</div>";
	}
	echo $str;
	exit;
