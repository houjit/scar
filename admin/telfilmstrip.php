...
$m_name = '手机版轮播广告管理';
//允许操作
$ac_arr = array('list'=>'广告列表','add'=>'添加广告','edit'=>'编辑广告','del'=>'删除广告','bulkdel'=>'批量删除','bulksort'=>'更新排序');
//当前操作
...
if ($ac == 'list')
{
    ...
    $Page = new Page($db->tb_prefix.'filmstrip');
    $list = $Page->get_data();
...
elseif ($ac == 'add' || $ac == 'edit')
{
    ...
    if (submitcheck('a'))
    {
        //$arr_not_empty = array('upload'=>'图片不可为空');
        //can_not_be_empty($arr_not_empty,$_POST);
        foreach (array('url','orderid') as $v)
        {
    ...
    {
        if (empty($_GET['id'])) $data = array('id'=>'','pic'=>'','detail'=>'','orderid'=>'');
