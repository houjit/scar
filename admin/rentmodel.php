...
if ($ac == 'list')
{
    $list = $db->row_select('rentmodel','1=1','*',0,'orderid asc');
	$tpl->assign( 'sortlist', $list );
    $tpl->display( 'admin/rentmodel_list.html' );
    exit;
}
//单条删除
elseif ($ac == 'del')
{
    ...
    $rs = $db->row_delete('rentmodel',"s_id=$s_id");
}
//批量删除
elseif ($ac == 'bulkdel')
{
    ...
    $rs = $db->row_delete('rentmodel',"s_id in($str_id)");
}
//批量排序
elseif ($ac == 'bulksort')
{
    ...
    {
        $rs = $db->row_update('rentmodel',array('orderid'=>$_POST['orderid'][$v]),"s_id=".intval($v));
    }
}
//添加
elseif ($ac == 'add' || $ac == 'edit')
{
    ...
    {	
        ...
        if ($ac == 'add')
        {
            $rs = $db->row_insert('rentmodel',$post);
        }
        else
		{ 	
			$rs = $db->row_update('rentmodel',$post,"s_id=".intval($_POST['s_id']));
		}
    }
    //转向添加或修改页面
    else 
    {
        ...
        else $data = $db->row_select_one('rentmodel',"s_id=".intval($_GET['s_id']));
		$tpl->assign( 'sort', $data );
		$tpl->assign( 'ac', $ac );
        $tpl->display( 'admin/add_rentmodel.html' );
        exit;
    }
}
//默认操作
